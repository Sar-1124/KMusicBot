...
public class ForceRemoveCmd extends DJCommand
{
    public ForceRemoveCmd(Bot bot)
    {
        ...
        this.help = "대기열에서 사용자가 추가한 모든 항목을 제거해요.";
        this.arguments = "<유저>";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyError("대기열에 아무것도 존재하지 않아요.");
            return;
        }
    ...
    private void removeAllEntries(User target, CommandEvent event)
    {
        ...
        if (count == 0)
        {
            event.replyWarning("**"+target.getName()+"** 대기열에 노래가 없어요!");
        }
        else
        {
            event.replySuccess("**"+target.getName()+"#"+target.getDiscriminator()+"**님의 목록"+"`"+count+"`개를 제거했어요."); //
        }
    }
}
