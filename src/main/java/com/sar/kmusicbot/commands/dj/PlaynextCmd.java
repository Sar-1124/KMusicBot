...
public class PlaynextCmd extends DJCommand
{
    ...
    public PlaynextCmd(Bot bot)
    {
        ...
        this.name = "playnext";
        this.arguments = "<곡 제목|URL>";
        this.help = "바로 다음에 재생될 노래를 등록해요.";
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        {
            event.replyWarning("노래 제목이나 URL을 포함하세요.");
            return;
        }
    ...
    private class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track)
        {
            ...
            {
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" 이 트랙은 (**"+track.getInfo().title+"**) 허용된 최대 길이보다 길어요: `"
                        +FormatUtil.formatTime(track.getDuration())+"` > `"+FormatUtil.formatTime(bot.getConfig().getMaxSeconds()*1000)+"`")).queue();
                return;
            }
            AudioHandler handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
            int pos = handler.addTrackToFront(new QueuedTrack(track, event.getAuthor()))+1;
            String addMsg = FormatUtil.filter(event.getClient().getSuccess()+" 추가됨 **"+track.getInfo().title
                    +"** (`"+FormatUtil.formatTime(track.getDuration())+"`) "+(pos==0?"바로 재생할게요!":+pos)+"번째 대기열에 추가되었어요.");
        ...
        {
            if(ytsearch)
                m.editMessage(FormatUtil.filter(event.getClient().getWarning()+" 검색결과가 없어요. `"+event.getArgs()+"`.")).queue();
            else
        ...
        public void loadFailed(FriendlyException throwable)
        {
            if(throwable.severity==FriendlyException.Severity.COMMON)
                m.editMessage(event.getClient().getError()+" 로딩중 오류가 발생했어요: "+throwable.getMessage()).queue();
            else
                m.editMessage(event.getClient().getError()+" 트랙 로딩중 오류가 발생했어요.").queue();
        }
    }
}
