 ...
 */
package com.sar.kmusicbot.utils;
...
{
    public final static String NEW_VERSION_AVAILABLE = "사용 가능한 KMusicBot의 새 버전이 있습니다.!\n"
                    + "현재 버전: %s\n"
                    + "새 버전: %s\n\n"
                    + "https://github.com/sar-1124/KMusicBot/releases/latest 에서 최신 릴리즈를 받으십시오.";
    ...
    public static Path getPath(String path)
    {
        ...
        if(path.toLowerCase().startsWith(WINDOWS_INVALID_PATH))
        {
            ...
            try
            {
                path = new File(KMusicBot.class.getProtectionDomain().getCodeSource().getLocation().toURI()).getParentFile().getPath() + File.separator + filename;
            }
    ...
    {
        if(KMusicBot.class.getPackage()!=null && KMusicBot.class.getPackage().getImplementationVersion()!=null)
            return KMusicBot.class.getPackage().getImplementationVersion();
        else
            return "UNKNOWN";
    }
    ...
    public static String getLatestVersion()
    {
        try
        {
            Response response = new OkHttpClient.Builder().build()
                    .newCall(new Request.Builder().get().url("https://api.github.com/repos/sar-1124/KMusicBot/releases/latest").build())
                    .execute();
