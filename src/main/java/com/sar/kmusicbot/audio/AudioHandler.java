 ...
 */
package com.sar.kmusicbot.audio;

import com.github.natanbc.lavadsp.karaoke.KaraokePcmAudioFilter;
import com.github.natanbc.lavadsp.timescale.TimescalePcmAudioFilter;
import com.github.natanbc.lavadsp.tremolo.TremoloPcmAudioFilter;
import com.sar.kmusicbot.Bot;
import com.sar.kmusicbot.KMusicBot;
import com.sar.kmusicbot.playlist.PlaylistLoader;
...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler 
{
    ...
    public Message getNowPlaying(JDA jda)
    {
        if(isMusicPlaying(jda))
        {
            ...
            mb.append(FormatUtil.filter(manager.getBot().getConfig().getSuccess()+" **"+guild.getSelfMember().getVoiceState().getChannel().getAsMention()+"**을 재생중이에요."));
            ...
            eb.setDescription((audioPlayer.isPaused() ? KMusicBot.PAUSE_EMOJI : KMusicBot.PLAY_EMOJI)
    ...
    public Message getNoMusicPlaying(JDA jda)
    {
        ...
        return new MessageBuilder()
                .setContent(FormatUtil.filter(manager.getBot().getConfig().getSuccess()+" **지금재생중...**"))
                .setEmbed(new EmbedBuilder()
                .setTitle("음악이 재생되지 않고있어요.")
                .setDescription(KMusicBot.STOP_EMOJI+" "+FormatUtil.progressBar(-1)+" "+FormatUtil.volumeIcon(audioPlayer.getVolume()))
    ...
    public String getTopicFormat(JDA jda)
    {
        if(isMusicPlaying(jda))
        {
            ...
            return "**"+title+"** ["+(userid==0 ? "autoplay" : "<@"+userid+">")+"]"
                    + "\n" + (audioPlayer.isPaused() ? KMusicBot.PAUSE_EMOJI : KMusicBot.PLAY_EMOJI) + " "
                    + "[" + FormatUtil.formatTime(track.getDuration()) + "] "
                    + FormatUtil.volumeIcon(audioPlayer.getVolume());
        }
        else return "음악이 재생되지 않고있어요.. " + KMusicBot.STOP_EMOJI + " " + FormatUtil.volumeIcon(audioPlayer.getVolume());
    }
